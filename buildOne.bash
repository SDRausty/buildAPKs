...
_SBOTRPERROR_() { # Run on script error.
	...
	then 
		mkdir -p "$RDR/var/tmp"
		if [[ -n $(find "$RDR/var/tmp" -name "*.err") ]] # https://unix.stackexchange.com/questions/46541/how-can-i-use-bashs-if-test-and-find-commands-together
		then 
			:
		else
			CER="conf.$NUM.err"
...
printf "\\e[1;38;5;115mCopied to Download/builtAPKs/%s/step%s.apk\\n" "$EXT$DAY" "$NOW"
printf "\\e[1;38;5;149mIt can be installed from Download/builtAPKs/%s/step%s.apk\\n" "$EXT$DAY" "$NOW" 
