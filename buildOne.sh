...
_STRPERROR_() { # Run on script error.
	local RV="$?"
	printf "\\e[?25h\\e[1;7;38;5;0mbuildAPKs buildOne.sh ERROR:  Signal %s received!\\e[0m\\n" "$?"
	if [[ "$RV" = 255 ]] 
	then 
...
_STRPSIGNAL_() { # Run on signal.
	printf "\\e[?25h\\e[1;7;38;5;0mbuildAPKs buildOne.sh WARNING:  Signal %s received!\\e[0m\\n" "$?"
 	exit 211 
}

_STRPQUIT_() { # Run on quit.
	printf "\\e[?25h\\e[1;7;38;5;0mbuildAPKs buildOne.sh WARNING:  Quit signal %s received!\\e[0m\\n" "$?"
 	exit 221 
}
